...
Den biologiske adaptasjon  til styrketrening hos mennesker varierer med faktorer som volum, intensitet, hvile mellom repetisjoner,  antall set, utvalg og rekkefølge på øvelser og hyppighet av styrkeøkter. (Ratamess et al.2009) Genetisk variasjon og miljøfaktorer  er også medvirkende årsaker til  variasjonen i adaptasjon. (Timmons,2011; Morton et al. 2018; Seaborne et al. 2018)
...
Uansett årsak til at man trener, er det ønskelig med  størst mulig effekt sett i forhold til innsats og for de fleste gjelder dette også tid. Jeg  ønsker å se om det er en forskjell i økning av muskelstyrke ved å gjennomføre  styrketrening i ett sett hhv  flere sett, hvilket vil være tidsbeasparende, og lettere for "folk flest" å kunne gjennomføre. Min  hypotese  er at trening med tre sett vil gi et større respons enn trening med  ett sett.
...
### Resultater:
#### Kroppssammensetning
Det ses  en signifikant endring i  fettfri masse i bein  i gruppen som gjennomførte tre sett 3.32% (± 4.39%) sammenlignet med de beina som gjennomførte ett sett 2.04% (± 3.71%). Statistisk signifikansverdi p < 0.05.
...
#| warning: false
#| message: false
#| tbl-cap: "Sammenligner endringer i fettfri masse hos deltakerne og lager figur"
...
#| message: false
#| tbl-cap: "ANCOVA"
#| tbl-subcap: "Lager datasett med endring og tilpasser model"


dat <- strengthvolume %>%
  filter(exercise == c("legpress","legext") %>% 
         time %in% c("pre", "post")) %>%
  pivot_wider(names_from = time, 
              values_from = load) %>%
...
m2 <- lm(change ~ pre + sets, data = dat)
...
strengthvolume %>%
  ...
  labs(title = "Estimater fra en ANCOVA modell", 
       subtitle = "post ~ pre + sets", 
       color = "Sets") + theme_classic()
```
...
bind_rows(data.frame(emmeans(m0, specs = ~ sets)) %>%
        mutate(Model = "post only"), 
        data.frame(emmeans(m2, specs = ~ sets)) %>%
        mutate(Model = "ANCOVA")) %>%
...
str.mod %>%
 ...
 ggplot(aes(time, combined.load, group = paste(participant, sets), color = sets)) + geom_line() + theme_bw() + labs(title = "Figur 2: Endring i styrke",
       y = "Endring i styrke (combined load)",
       x = "Time",
...
m4 <- lm(post ~ pre + sets, data = str.dat)

m5 <- lm(change ~ pre + sets, data = str.dat)
...
m6 <- aov(post ~ pre + sets, data = str.dat)
...
strengthvolume %>%
  ...
  geom_abline(intercept = coef(m1)[1], slope = coef(m1)[2], color = "red")  + 
  geom_abline(intercept = coef(m1)[1] + coef(m1)[3], slope = coef(m1)[2], color = "blue") + 
  labs(title = "Estimater fra en ANCOVA modell - styrke", 
       subtitle = "post ~ pre + sets", 
       color = "sets") + theme_classic()
```
...
bind_rows(data.frame(emmeans(m0, specs = ~ sets)) %>%
        mutate(Model = "Post only"), 
        data.frame(emmeans(m4, specs = ~ sets)) %>%
        mutate(Model = "ANCOVA")) %>%
        ggplot(aes(sets, emmean, fill = Model)) + 
        geom_errorbar(aes(ymin = emmean, ymax = emmean + SE), 
                      position = position_dodge(), width = 0.5) +
...
Den totale treningsbelastingen i multiple-set-gruppen er  høyere  enn i single-set-gruppen, og man vil også forvente en dosis-respons økning i  både styrke og muskelmasse.
Vår studie benytter  within-subject-design, hvor  hver  deltaker er sin egen kontroll.  Med dette reduserer vi risiko for bias. 


### Konklusjon

Våre funn  støtter hypotesen  vå om av det gir et bedre respons på styrketrening og muskelmasse med tre sett styrketrening  sammenlignet med ett sett. 
